...
const data = [
    { study_load: 0, average_self_esteem: 17.35 },
    { study_load: 1, average_self_esteem: 24.88 },
    { study_load: 2, average_self_esteem: 22.50 },
    { study_load: 3, average_self_esteem: 15.55 },
    { study_load: 4, average_self_esteem: 9.10 },
    { study_load: 5, average_self_esteem: 9.76 }
];
...
yScale.domain([0, d3.max(data, d => d.average_self_esteem)]);
...
svg.append("text")
    ...
    .attr("dy", "1em")
    .attr("class", "axis-label") // Use classed() to add the class
    .text("Average Self Esteem");

// Title of graph
svg.append("text")
    .attr("x", width / 2) // Set the x-coordinate to the center of the SVG
    .attr("y", -margin.top / 2) // Adjust the y-coordinate as needed
    .attr("class", "axis-label")
    .attr("text-anchor", "middle") // Center the text horizontally
    .attr("style", "font-size: 20px;")
    .text("Average Self Esteem vs Study Load");


const colorScale = d3.scaleSequential(d3.interpolateRgb)
    .domain([d3.max(data, d => d.average_self_esteem), 0])
...
const bars = svg.selectAll(".bar")
    ...
    .attr("width", barWidth)
    .attr("y", d => yScale(d.average_self_esteem))
    .attr("height", d => height - yScale(d.average_self_esteem))
    .style("fill", d => colorScale(d.average_self_esteem))
...
bars.append("text")
    ...
    .text(function (d) { return (d.average_self_esteem); });  
...
function handleMouseOver(d, i) {
    ...
    d3.select(this)
        ...
        .attr("y", d => yScale(d.average_self_esteem) - 10)
        .attr("height", d => height - yScale(d.average_self_esteem) + 10);
